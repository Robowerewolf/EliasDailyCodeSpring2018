...
enum MYKEYS {
	KEY_UP, KEY_DOWN, KEY_LEFT, KEY_RIGHT, KEY_M, KEY_W, KEY_A, KEY_S, KEY_D, KEY_SPACE
};
enum DIRECTIONS {
	RIGHT, LEFT, UP, DOWN
};
...
int main() {
	...
	//float player2_x = SCREEN_W - player2_SIZE / 2.0;
	//float player2_y = SCREEN_H - player2_SIZE / 2.0;
	...
	while (!doexit) {
		...
		if (ev.type == ALLEGRO_EVENT_TIMER) {
			...
			//if (key[KEY_W] && !wallCollide(player2_x, player2_y, UP, angle, 32, level)) {
			//	//player1_y -= 4.0;
			//	player2_y += 4 * sin(3.14*angle / 180);
			//	player2_x += 4 * cos(3.14*angle / 180);
			//}

			//if (key[KEY_S] && !wallCollide(player2_x, player2_y, DOWN, angle, 32, level)) {
			//	//player1_y += 4.0;
			//	player2_y -= 4 * sin(3.14*angle / 180);
			//	player2_x -= 4 * cos(3.14*angle / 180);
			//}
			...
			//		//	cout << "checking bullet" << endl;
			//		if (i->isDead() && justFired > 20) {
			//			i->fire(player2_x, player2_y, angle + 180);
			//			//		cout << "fired: ";
			//			i->printInfo();
			//			justFired = 0;
			//			break;
		...
		if (redraw && al_is_event_queue_empty(event_queue)) {
			...
			//al_draw_rotated_bitmap(player2, 16, 16, player2_x, player2_y, ((angle*3.14) / 180), 0);
			////draw live bullets
			for (i = ammo.begin(); i < ammo.end(); i++)
				if (i->isDead() == false)
					i->drawBullet();
